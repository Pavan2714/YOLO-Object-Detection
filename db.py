"""Database utility functions for YOLOv11."""
...
class User(UserMixin):
    """User class for Flask-Login."""
    def __init__(self, user_id, username, email, profile_image=None, google_id=None, auth_provider='local'):
        self.id = user_id
    ...
    def get(user_id):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, username, email, profile_image, google_id, auth_provider "
                "FROM users WHERE id = ?",
                (user_id,)
            )
            ...
            return User(
                user['id'], user['username'], user['email'], user['profile_image'],
                user['google_id'], user['auth_provider']
            )
...
def get_user_videos(_user_id):
    """Placeholder for fetching user videos. Not yet implemented."""
